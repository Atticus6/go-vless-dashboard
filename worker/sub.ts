...
// 订阅限流：按客户端 IP 计数，key 为 `sub-ip:<cf-connecting-ip>`。
// 配额见 wrangler.jsonc 的 ratelimits（默认 60 次/60 秒/边缘位置），调参只改配置即可.
// 挂在 subApp 的 /:token 路径上，进 handler（D1 查询）之前超限直接 429.
const subRateLimit = factory.createMiddleware(async (c, next) => {
  // cf-connecting-ip 由 Cloudflare 边缘注入，客户端无法伪造或去掉.
  const ip = c.req.header("cf-connecting-ip") ?? "unknown";
  const limiter = c.env.SUB_RATE_LIMITER;
  // 绑定缺失（如本地直跑未挂 binding）时放行并告警，避免订阅整体不可用.
  if (!limiter) {
    console.warn(
      JSON.stringify({ message: "SUB_RATE_LIMITER binding missing, skip" }),
    );
    return next();
  }
  const allowed = (
    await limiter.limit({ key: `sub-ip:${ip}` })
  ).success;
  if (!allowed) {
...
// subRateLimit 挂在 /:token 上：按客户端 IP 限流，超限在进 handler 前 429.
const subApp = factory
  .createApp()
  .use("/:token")
  .get(
    "/:token",
    ...
    async (c) => {
      ...
      const format =
        query.format ?? detectSubFormat(c.req.header("user-agent"));
      ...
      const scoped = myScope ? owned.filter((n) => myScope.has(n.id)) : owned;
      ...
      // Clash：服务端先把 18 个 ruleset 拉下来内联进 rules，一次返回完整配置；
